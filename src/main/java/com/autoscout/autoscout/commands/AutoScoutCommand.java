...
public class AutoScoutCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        ...
        builder.then(literal("stop")
            .executes(context -> {
                if (ElytraController.isActive()) {
                    ...
                    info("Auto Scout flight stopped.");
                } else {
                    error("Auto Scout is not currently active.");
                }
                return SINGLE_SUCCESS;
            })
        );
        ...
        builder.then(literal("resume")
            .executes(context -> {
                if (ElytraController.isActive()) {
                    error("Auto Scout is already active.");
                } else {
                    ElytraController.resume();
                    info("Auto Scout flight resumed.");
                }
                return SINGLE_SUCCESS;
            })
        ...
        builder.then(literal("list")
            .executes(context -> {
                info("Saved trips:");
                for (com.autoscout.autoscout.utils.TripManager.TripData trip : com.autoscout.autoscout.utils.TripManager.getTrips()) {
                    info("  " + trip.timestamp);
                }
                return SINGLE_SUCCESS;
            })
        ...
        builder.then(literal("help")
            .executes(context -> {
                info("Auto-Scout Commands:");
                info("§7/AutoScout start <x1> <z1> <x2> <z2> [stripWidth] §f- Start scanning an area");
                info("§7/AutoScout stop §f- Stop the current scanning operation");
                info("§7/AutoScout pause §f- Pause the current scanning operation");
                info("§7/AutoScout resume [timestamp] §f- Resume the latest or a specific trip");
                info("§7/AutoScout list §f- List all saved trips");
                info("§7/AutoScout status §f- Show current status and progress");
                info("§7/AutoScout repair §f- Show elytra repair status");
                info("§7/AutoScout repair toggle §f- Toggle auto elytra repair");
                info("§7/AutoScout help §f- Show this help message");
                info("");
                info("§eCoordinate Examples:");
                info("§f/AutoScout start 1000 1000 5000 5000 150 §7- Absolute coordinates");
                info("§f/AutoScout start ~ ~ ~4000 ~4000 150 §7- Relative to current position");
                info("§f/AutoScout start ~-2000 ~-2000 ~2000 ~2000 120 §7- Relative with offsets");
                info("");
        ...
        builder.executes(context -> {
            info("Use §7/AutoScout help §ffor command usage.");
            return SINGLE_SUCCESS;
        });
    }
    /**
    ...
    private int startScanning(int x1, int z1, int x2, int z2, int stripWidth) {
        if (ElytraController.isActive()) {
            error("Auto Scout is already active! Use '/AutoScout stop' first.");
            return SINGLE_SUCCESS;
        }
